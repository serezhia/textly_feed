...
FutureOr<Response> _get(RequestContext context, String id) async {
  ...
  try {
    logger.d(
      '$uuid: Read users comments, userId: $userId, reqUserId: $reqUserId',
    );
    final chunk = await feedRepository.readUserComments(
      userId: userId,
      reqUserId: reqUserId,
      offset: offset,
      limit: limit,
    ...
    logger.d(
      '$uuid: Readed users comments, userId: $userId, reqUserId: $reqUserId',
    );
    return TextlyResponse.success(
      uuid: uuid,
      message:
          'Success read users comments userId: $userId, reqUserId: $reqUserId',
      data: chunk.toJson(),
    );
  } catch (e) {
    logger.e(
      '''$uuid: Error read users comments , userId: $userId, reqUserId: $reqUserId''',
    );
    return TextlyResponse.error(
      uuid: uuid,
      statusCode: 500,
      message: 'Error read users comments',
      error: '$e',
      errorCode: 0,
    );
  }
